...
def main(problem, ws_qa_pairs, know_qa_pairs, psl):
        ...
        if len(k_ans_list)==0:
            ...
            # If q1=T, q2=T, q3=T
                ...
                for (know_choice,verb2) in choice1_set_of_sim_ans:
                    new_ans = ""
                    psl["context"].append(prob['choice1']+'$$'+know_choice)
                    for ans_token in ans_tokens:
                ...
                for (know_choice,verb2) in choice2_set_of_sim_ans:
                    psl["context"].append(prob['choice2']+'$$'+know_choice)
                    if know_choice == "":
                        continue
                    new_ans = ""
                    for ans_token in ans_tokens:
                ...
                for (k_ans,k_verb) in k_ans_list:
                    psl["context"].append(prob['pronoun']+'$$'+k_ans)
                    if k_ans=="":
                        continue
    ...
    for (t,h,p) in ent_comparisons_for_choice1:
        if are_same(h.lower(),p.lower()):
            ...
            if use_predictor:
                ...
                psl["entailment"].append(prob['choice1']+'$$'+prob['pronoun']+'$$'+str(ent_score))
            else:
                ent_score = 0.0
                cntr_score = 0.0
    ...
    for (t,h,p) in ent_comparisons_for_choice2:
        if are_same(h.lower(),p.lower()):
            ...
            if use_predictor:
                ...
                ent_score = score["label_probs"][0]
                cntr_score = score["label_probs"][1]
                psl["entailment"].append(prob['choice2']+'$$'+prob['pronoun']+'$$'+str(ent_score))
            else:
                ent_score = 0.0
                cntr_score = 0.0
        ...
        if ws_sent in qasrl_output_dict.keys():
            ...
            if "know_sent" in prob:
                ...
                if know_s in qasrl_output_dict.keys():
                    ...
                    print("Before WS_PAIR "+str(ws_sent_qasrl_pairs))
                    print("Before KNOW_PAIR "+str(know_sent_qasrl_pairs))
                    ...
                    #WS_PAIR [{'ques': 'What V something ?', 'ans': 'The fish', 'verb': 'ate'}, {'ques': 'What did something V ?', 'ans': 'the worm', 'verb': 'ate'}, {'ques': 'What V something ?', 'ans': 'It', 'verb': 'was'}, {'ques': 'What did something V ?', 'ans': 'hungry', 'verb': 'was'}]
                    #KNOW_PAIR [{'ques': 'Who V something ?', 'ans': 'I', 'verb': 'ate'}, {'ques': 'What did someone V ?', 'ans': 'something', 'verb': 'ate'}, {'ques': 'Why did someone V something ?', 'ans': 'because im hungry', 'verb': 'ate'}, {'ques': 'Who V something ?', 'ans': 'I', 'verb': 'am'}, {'ques': 'Who V something ?', 'ans': 'i', 'verb': 'am'}, {'ques': 'What does someone V ?', 'ans': 'sleepy', 'verb': 'am'}]
                    if verbose_output:
                        print("************************************************************")
